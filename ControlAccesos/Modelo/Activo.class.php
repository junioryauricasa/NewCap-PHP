...
class Activo{
	...
	function ObtenerActivos()
	{
		...
		$query="SELECT idActivo,codigo,descripcion,estado
                FROM activo
                WHERE estado=1 and idActivo>1";

		$arreglo = mysql_query($query);
    	return $arreglo;	
	}
	function CrearActivo($CodigoCustodio,$NombreCustodio)
    {
        $this->con->conectar(); 

        $query = "insert into activo(codigo,descripcion,estado)
	...
	function EliminarActivo($idActivo)
    {
        ...
        $query = "UPDATE activo SET ESTADO = 0 
