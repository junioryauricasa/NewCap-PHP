...
class Ejecutivo{
	...
	function ObtenerEjecutivos()
	{
		...
		$query="SELECT idEjecutivos,nombre,comision,estado
                FROM ejecutivos
                WHERE estado=1";
	...
	function CrearEjecutivo($NombreEjecutivo,$ValorComision)
    {
        $this->con->conectar(); 

        $query = "insert into ejecutivos(nombre,comision,estado)
                values('".$NombreEjecutivo."',".$ValorComision.",1)";
        mysql_query($query);
            $var = mysql_insert_id();
            return  $var;
    }
	
	function EliminarEjecutivo($idEjecutivo)
    {
        $this->con->conectar(); 

        $query = "UPDATE ejecutivos SET ESTADO = 0 
        WHERE idEjecutivos = " .$idEjecutivo."";
        //echo var_dump($query);
